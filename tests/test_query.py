...
def _client(
    config: Config, plan_cache: PlanCache, ollama: OllamaClient
) -> TestClient:
    app = create_app(config, plan_cache, ollama=ollama)
...
def test_query_with_known_agent_returns_resolved_groups(
    ...
    app = create_app(baseline_config, plan_cache, ollama=mock_ollama)
...
def test_healthz_remains_public_after_query_route_added(
    ...
    with _client(baseline_config, plan_cache, mock_ollama) as client:
